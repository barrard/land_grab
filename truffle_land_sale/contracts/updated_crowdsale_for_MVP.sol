...
contract Crowdsale {
  ...
  // Aprice per token
  uint256 public price_per_token = 18 finney;
...
//   }
   ...
   */
  function buyTokens(address _beneficiary, uint _token_amount) public payable {

    // uint256 weiAmount = msg.value;
    // uint256 tokenAmount = _token_amount;
    // _preValidatePurchase(_beneficiary, tokenAmount);
    //     // function buy_x_tokens(uint _token_amount) public payable returns (uint){
    //     uint eightneen = 18;
    //     uint oneHundren = 100;
    //     uint perToken = eightneen.div(oneHundren);
    //   uint eth = _token_amount.mul(perToken);
    //   return eth;
    // }
    ...
    _updatePurchasingState(_beneficiary, msg.value);

    _forwardFunds();
    _postValidatePurchase(_beneficiary, msg.value);
  }
...
contract CappedCrowdsale is Crowdsale {
  ...
  function _preValidatePurchase(address _beneficiary, uint256 _tokenAmount) constant internal {
    super._preValidatePurchase(_beneficiary, _tokenAmount);
    require(weiRaised.add(_tokenAmount.mul(price_per_token)) <= cap);
  }
...
// 43200, 1, "0x038343bfaf1f35b01d91513c8472764d55474045", "1000", "0x692a70d2e424a56d2c6c27aa97d1a86395877b3a", "1000"
  ...
  function SampleCrowdsale(uint256 _crowdsale_length_minutes, uint256 _rate, address _wallet, uint256 _cap, ERC20 _token, uint256 _goal) public
    ...
    RefundableCrowdsale(_goal)
  {
    ...
    //     super._postValidatePurchase(weiRaised, goal);
    //   }
    //   return (_beneficiary, _tokenAmount);
    // }

    // function _checkIfCrowdsaleGoalReached () public returns(bool res) {
    //   if(weiRaised == goal) return true;
    //   return false;
    // }
    function buy_1_token(uint _token_amount) public payable returns (bool _is_true, uint _price, uint value_sent, uint total_needed){
        // uint price = 18 finney;
        uint price = price_per_token;
      require(msg.value== price.mul(_token_amount));
        buyTokens(msg.sender, msg.value);
        return (true, price, msg.value, price.mul(_token_amount));

    }
    function buy_x_tokens(uint _token_amount) public returns (uint){
        uint eightneen = 18;
        uint oneHundren = 100;
        uint perToken = eightneen.div(oneHundren);
      uint eth = 1 finney * _token_amount.mul(perToken);
      return eth;
    }
    ...
    function FORCE_End_crowd_sale () public onlyOwner returns(bool res) {
      closingTime = now;
      return true;
    }
}
